...
const generatePlate = (text: string, index: number): Promise<ImageFile> => {
  return new Promise((resolve, reject) => {
    ...
    const width = 512; // Square canvas to avoid influencing aspect ratio
    const height = 512;
    canvas.width = width;
    ...
    // Black text
    ctx.fillStyle = 'black';
    // Use a large, clear font. Arial is a safe fallback.
    ctx.font = 'bold 72px Arial';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.direction = 'rtl'; // Crucial for right-to-left languages like Persian
...
export const extractTextAndGeneratePlates = async (
  ...
  // Regex to find text inside double quotes, single quotes, or Persian quotes «»
  const regex = /"([^"]+)"|'([^']+)'|«([^»]+)»/g;
  const matches = [...prompt.matchAll(regex)];
  // Extract the captured group (the text itself)
  const texts = matches.map((match) => match[1] || match[2] || match[3]).filter(Boolean);

...
// FIX: Update imports to use the more complete types from src/types.ts
import {
  AspectRatio,
  GenerateVideoParams,
  GenerationMode,
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [model, setModel] = useState<VeoModel>(
    (initialValues?.model as VeoModel) ?? VeoModel.VEO_FAST,
  );
  const [aspectRatio, setAspectRatio] = useState<AspectRatio>(
    (initialValues?.aspectRatio as AspectRatio) ?? AspectRatio.LANDSCAPE,
  );
  const [resolution, setResolution] = useState<Resolution>(
    initialValues?.resolution ?? Resolution.P720,
  );
  const [generationMode, setGenerationMode] = useState<GenerationMode>(
    (initialValues?.mode as GenerationMode) ?? GenerationMode.TEXT_TO_VIDEO,
  );
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setModel((initialValues.model as VeoModel) ?? VeoModel.VEO_FAST);
      setAspectRatio(
        (initialValues.aspectRatio as AspectRatio) ?? AspectRatio.LANDSCAPE,
      );
  ...
  const handleSelectMode = (mode: GenerationMode) => {
    ...
    setStartFrame(null);
    setEndFrame(null);
    // Do not reset reference images as they might come from the engineering step
    // setReferenceImages([]);
    setStyleImage(null);
    setInputVideo(null);
  ...
  const renderMediaUploads = () => {
    const commonMediaContainerClasses =
      'mb-3 p-4 bg-[#2c2c2e] rounded-xl border border-gray-700 flex flex-wrap items-center justify-center gap-2';

    if (generationMode === GenerationMode.FRAMES_TO_VIDEO) {
      return (
        <div className={commonMediaContainerClasses}>
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          <div className="relative" ref={modeSelectorRef}>
            <button
              ...
              aria-label="Select generation mode">
              {
                {
                  [GenerationMode.TEXT_TO_VIDEO]: (
                    <TextModeIcon className="w-5 h-5" />
                  ),
                  [GenerationMode.FRAMES_TO_VIDEO]: (
          ...
          <button
            ...
            className={`p-2.5 rounded-full hover:bg-gray-700 ${
